...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!identity || !hasTestPermission(identity, "NOTIFICATION_READ", testScope)) return errorResponse("AUTHORIZATION_DENIED", requestId, 403);
    return Response.json({ notifications: getLocalAspirationService().listNotifications(testScope.periodCode) }, { headers: { "x-request-id": requestId } });
  } catch {
